...
Edited by Radon Rosborough for personal use.
...
class Button():
    def __init__(self, p1, p2, text='Button'):
        self.rect = Rectangle(p1, p2)
        self.lbl = Text(self.rect.getCenter(), text)
        self.active = True
    def draw(self, win):
        self.rect.draw(win)
        self.lbl.draw(win)
    ...
    def clicked(self, click):
        if click:
            p1 = self.rect.getP1()
            p2 = self.rect.getP2()
            if (click.getX() >= p1.getX() and
               click.getX() <= p2.getX() and
               click.getY() >= p1.getY() and
               click.getY() <= p2.getY() and
               self.active):
               return True
        return False
    def activate(self):
    ...
    def setActive(self, active):
        self.active = active
    def getLabel(self):
        return self.lbl.getText()

class Checkbox():
    def __init__(self, p, checked):
        self.p1 = Point(p.x-10, p.y-10)
        self.p2 = Point(p.x+10, p.y+10)
        self.rect = Rectangle(self.p1, self.p2)
        self.checked = checked
        self.imageoff = Image(p, 'checkboxoff.gif')
        self.imageon = Image(p, 'checkboxon.gif')
        self.imagecheck = Image(p, 'checkboxcheck.gif')
        self.drawn = 'off'
    def draw(self, win):
        if self.checked:
            self.imageon.draw(win)
        else:
            self.imageoff.draw(win)
    def check(self, click, win):
       if click:
           p1 = self.rect.getP1()
           p2 = self.rect.getP2()
           if (click.getX() >= p1.getX() and
               ...
               click.getY() >= p1.getY() and
               click.getY() <= p2.getY()):
               if self.checked: self.checkoff(win)
               else: self.checkon(win)
               return True
       return False
    def checkflip(self, win):
        if self.checked: self.checkoff(win)
        else: self.checkon(win)
    def checkoff(self, win):
        if self.drawn != 'off':
            try:
               self.imageon.undraw()
               ...
               pass
            try:
               self.imagecheck.undraw()
            except GraphicsError:
               pass
            self.imageoff.draw(win)
            self.drawn = 'off'
            self.checked = False
    def checkon(self, win):
        if self.drawn != 'on':
            try: self.imageoff.undraw()
            except GraphicsError: pass
            try: self.imagecheck.undraw()
            except GraphicsError: pass
            self.imageon.draw(win)
            self.drawn = 'on'
            self.checked = True
    def checkcheck(self, win):
        if self.drawn != 'check':
            try: self.imageoff.undraw()
            except GraphicsError: pass
            try: self.imageon.undraw()
            except GraphicsError: pass
            self.imagecheck.draw(win)
            self.drawn = 'check'
            self.checked = False
